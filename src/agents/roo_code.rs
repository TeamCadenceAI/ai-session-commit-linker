//! Roo Code log discovery.
//!
//! Roo Code uses the same task storage model as Cline, with Roo-specific
//! extension IDs and backup folders.

use std::path::{Path, PathBuf};

use super::{
    AgentExplorer, SessionLog, SessionSource, app_config_dir_in, home_dir, recent_files_with_exts,
};
use crate::scanner::AgentType;
use async_trait::async_trait;

const ROO_EXT_IDS: &[&str] = &["rooveterinaryinc.roo-cline", "roocode.roo-code"];
const IDE_APPS: &[&str] = &["Code", "Cursor", "Windsurf", "VSCodium"];

pub async fn all_log_dirs() -> Vec<PathBuf> {
    let home = match home_dir() {
        Some(h) => h,
        None => return Vec::new(),
    };
    log_dirs_in(&home).await
}

pub struct RooCodeExplorer;

#[async_trait]
impl AgentExplorer for RooCodeExplorer {
    async fn discover_recent(&self, now: i64, since_secs: i64) -> Vec<SessionLog> {
        let dirs = all_log_dirs().await;
        recent_files_with_exts(&dirs, now, since_secs, &["json"])
            .await
            .into_iter()
            .map(|file| SessionLog {
                agent_type: AgentType::RooCode,
                source: SessionSource::File(file.path),
                updated_at: Some(file.mtime_epoch),
                match_reasons: Vec::new(),
            })
            .collect()
    }
}

async fn log_dirs_in(home: &Path) -> Vec<PathBuf> {
    let mut dirs = Vec::new();

    for app in IDE_APPS {
        let global_storage = app_config_dir_in(app, home)
            .join("User")
            .join("globalStorage");
        for ext in ROO_EXT_IDS {
            collect_dirs_with_exts(
                &global_storage.join(ext).join("tasks"),
                &mut dirs,
                &["json"],
            )
            .await;
        }
    }

    for backup in [
        home.join(".roo").join("task-history"),
        home.join(".roo").join("tasks"),
    ] {
        collect_dirs_with_exts(&backup, &mut dirs, &["json"]).await;
    }

    dirs.sort();
    dirs.dedup();
    dirs
}

async fn collect_dirs_with_exts(root: &Path, results: &mut Vec<PathBuf>, exts: &[&str]) {
    let mut stack = vec![root.to_path_buf()];
    while let Some(dir) = stack.pop() {
        let mut entries = match tokio::fs::read_dir(&dir).await {
            Ok(entries) => entries,
            Err(_) => continue,
        };

        let mut has_match = false;
        while let Ok(Some(entry)) = entries.next_entry().await {
            let path = entry.path();
            let file_type = match entry.file_type().await {
                Ok(file_type) => file_type,
                Err(_) => continue,
            };
            if file_type.is_dir() {
                stack.push(path);
            } else if file_type.is_file()
                && !has_match
                && let Some(ext) = path.extension().and_then(|e| e.to_str())
                && exts.iter().any(|allowed| allowed.eq_ignore_ascii_case(ext))
            {
                has_match = true;
            }
        }

        if has_match {
            results.push(dir);
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    use tempfile::TempDir;

    #[tokio::test]
    async fn test_roo_log_dirs_collects_global_storage_and_backup_dirs() {
        let home = TempDir::new().unwrap();

        let task_dir = app_config_dir_in("Code", home.path())
            .join("User")
            .join("globalStorage")
            .join("rooveterinaryinc.roo-cline")
            .join("tasks")
            .join("task-1");
        tokio::fs::create_dir_all(&task_dir).await.unwrap();
        tokio::fs::write(task_dir.join("task-1.json"), "{}")
            .await
            .unwrap();

        let backup_dir = home.path().join(".roo").join("tasks").join("abc");
        tokio::fs::create_dir_all(&backup_dir).await.unwrap();
        tokio::fs::write(backup_dir.join("api_conversation_history.json"), "{}")
            .await
            .unwrap();

        let dirs = log_dirs_in(home.path()).await;

        assert!(dirs.contains(&task_dir));
        assert!(dirs.contains(&backup_dir));
    }

    #[tokio::test]
    async fn test_roo_log_dirs_ignores_non_json_files() {
        let home = TempDir::new().unwrap();

        let task_dir = app_config_dir_in("Code", home.path())
            .join("User")
            .join("globalStorage")
            .join("roocode.roo-code")
            .join("tasks")
            .join("task-1");
        tokio::fs::create_dir_all(&task_dir).await.unwrap();
        tokio::fs::write(task_dir.join("note.txt"), "nope")
            .await
            .unwrap();

        let dirs = log_dirs_in(home.path()).await;
        assert!(!dirs.contains(&task_dir));
    }
}
