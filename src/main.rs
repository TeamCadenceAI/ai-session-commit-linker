...
fn hook_post_commit_inner() -> std::result::Result<(), HookError> {
    ...
    if let Some(ref matched) = session_match {
        ...
        if scanner::verify_match(&metadata, &repo_root, &head_hash) {
            ...
            // Optionally encrypt — this is the enforced path
            let final_content = maybe_encrypt_note(&note_content, &recipient)
                .map_err(|e| HookError::GpgEncryptionFailed(format!("{}", e)))?;

            // Attach the note
            git::add_note(&head_hash, &final_content)?;
...
fn try_resolve_single_commit(
    ...
    // Optionally encrypt — in retry path, encryption failure is transient
    let final_content = match maybe_encrypt_note(&note_content, recipient) {
        Ok(c) => c,
        Err(e) => {
            eprintln!(
                "[ai-session-commit-linker] warning: encryption failed for commit {}: {}",
    ...
    if git::add_note(commit, &final_content).is_ok() {
        eprintln!(
